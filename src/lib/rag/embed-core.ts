...
// Embeddings core — src/lib/rag/embed-core.ts
//
// The embedding implementation WITHOUT the "server-only" import guard, so
// standalone scripts (scripts/embed-courses.ts, scripts/warm-demo.ts) can
// reuse the exact same code path — including manual L2 normalization and
// dimension assertion — instead of duplicating it. src/lib/rag/embed.ts is
// the thin server-only wrapper that app code must import.
//
