...
function Toggle({ checked }: { checked?: boolean }) {
  return (
    <span className={`relative inline-flex h-[22px] w-[38px] items-center rounded-full border transition ${checked ? "bg-[#2E3AFF] border-[#2E3AFF]" : "bg-white border-[color:var(--color-border-resting)]"}`}>
      <span className={`inline-block size-[16px] rounded-full bg-white shadow-sm transition ${checked ? "translate-x-[18px]" : "translate-x-[2px]"}`} />
    </span>
  );
}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-[16px]">
          ...
          <nav className="lg:col-span-3 lg:sticky lg:top-[24px] h-fit rounded-[16px] border border-[color:var(--color-border-resting)] bg-[color:var(--color-surface-1)] p-[8px] flex lg:flex-col gap-[4px] overflow-x-auto">
            {[
              ...
              <a key={s.id} href={`#${s.id}`} className={`shrink-0 rounded-full px-[14px] py-[8px] text-[13px] font-medium text-center ${s.active ? "bg-[#141210] text-white" : "text-muted-foreground hover:bg-[#FFFCF7] hover:text-foreground"}`}>
                {s.label}
              </a>
            ))}
          </nav>
          ...
          <div className="lg:col-span-9 flex flex-col gap-[16px]">
            ...
            <section id="account" className="rounded-[16px] border border-[color:var(--color-border-resting)] bg-[color:var(--color-surface-1)] p-[18px]">
              ...
              <div className="mt-[14px] flex gap-[14px] items-center rounded-[14px] bg-[#FFFCF7] border border-[color:var(--color-border-resting)] p-[14px]">
                <div className="size-[44px] rounded-full bg-[#141210] text-white grid place-items-center text-[16px] font-[700] shrink-0">{(user?.name ?? "O").slice(0, 1)}</div>
                ...
                <Link href="/profile" className="shrink-0 rounded-full bg-[#141210] text-white px-[14px] py-[7px] text-[12px] font-semibold">View profile</Link>
              </div>
              <div className="mt-[12px] grid grid-cols-1 sm:grid-cols-2 gap-[10px]">
                <div className="rounded-[12px] bg-white border border-[color:var(--color-border-resting)] px-[12px] py-[11px]"><p className="text-[10px] font-semibold tracking-[0.08em] uppercase text-muted-foreground">Member since</p><p className="text-[13px] font-medium mt-[4px]">{user ? new Date(user.createdAt).toLocaleDateString() : "—"}</p></div>
                <div className="rounded-[12px] bg-white border border-[color:var(--color-border-resting)] px-[12px] py-[11px]"><p className="text-[10px] font-semibold tracking-[0.08em] uppercase text-muted-foreground">Auth</p><p className="text-[13px] font-medium mt-[4px]">Email · NextAuth</p></div>
              </div>
            </section>
            ...
            <section id="learning" className="rounded-[16px] border border-[color:var(--color-border-resting)] bg-[color:var(--color-surface-1)] p-[18px]">
              ...
              <div className="mt-[14px] flex flex-col gap-[10px]">
                <div className="flex items-center justify-between rounded-[12px] bg-[#FFFCF7] border border-[color:var(--color-border-resting)] px-[14px] py-[12px]">
                  ...
                  <select defaultValue="5" className="rounded-full border border-[color:var(--color-border-resting)] bg-white px-[12px] py-[7px] text-[13px] font-medium"><option>5h / week</option><option>8h / week</option><option>10h / week</option></select>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-[10px]">
                  <label className="flex items-center justify-between rounded-[12px] bg-white border border-[color:var(--color-border-resting)] px-[14px] py-[12px] cursor-pointer">
            ...
            <section id="notifications" className="rounded-[16px] border border-[color:var(--color-border-resting)] bg-[color:var(--color-surface-1)] p-[18px]">
              ...
              <div className="mt-[14px] flex flex-col gap-[10px]">
                {[
                  ...
                  <label key={r.t} className="flex items-center justify-between gap-[12px] rounded-[12px] bg-[#FFFCF7] border border-[color:var(--color-border-resting)] px-[14px] py-[12px] cursor-pointer">
            ...
            {/* Privacy */}
            <section id="privacy" className="rounded-[16px] border border-[color:var(--color-border-resting)] bg-[#141210] text-[#FFF8ED] p-[18px]">
              ...
              <div className="mt-[14px] flex flex-wrap gap-[8px]">
                <button className="rounded-full bg-white text-[#141210] px-[14px] py-[8px] text-[13px] font-semibold">Export my data</button>
                <button className="rounded-full border border-white/20 bg-white/10 px-[14px] py-[8px] text-[13px] font-medium">Delete assessment history</button>
