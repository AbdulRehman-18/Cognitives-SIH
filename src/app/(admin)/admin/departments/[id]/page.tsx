    ...
    <AppShell roleLabel="Admin" userName={session.user.name ?? session.user.email ?? "Admin"} nav={<AdminNav />}>
      <div className="mx-auto max-w-[980px] px-[20px] lg:px-[24px] py-[32px] flex flex-col gap-[20px]">
        ...
        <div className="rounded-[16px] border border-[color:var(--color-border-resting)] bg-[color:var(--color-surface-1)] flex flex-col sm:flex-row divide-y sm:divide-y-0 sm:divide-x divide-[color:var(--color-border-resting)] overflow-hidden">
          ...
          <div className="flex-1 px-[18px] py-[14px] bg-[color:var(--color-surface-1)]">
        ...
        {trainers.length > 0 && (
          <section className="rounded-[16px] border border-[#FDE68A]/40 bg-[#FFFBEB] p-[16px]">
            ...
            <div className="mt-[10px] flex flex-wrap gap-[8px]">
              {trainers.map((t) => (
                <span key={t.id} className="inline-flex items-center gap-[8px] rounded-full bg-[color:var(--color-surface-1)] border border-[#FDE68A] px-[12px] py-[7px] text-[13px] font-medium shadow-sm">
        ...
        {roles.length === 0 && learners.length === 0 ? (
          <div className="rounded-[16px] border border-dashed border-[color:var(--color-border-resting)] bg-[color:var(--color-surface-1)] px-[20px] py-[20px] text-center text-[14px] text-muted-foreground">No learners assigned to this division yet. Assign officers to see actual gaps.</div>
        ) : (
          roles.map((r) => (
            <section key={r.id} className="rounded-[16px] border border-[color:var(--color-border-resting)] bg-[color:var(--color-surface-1)] overflow-hidden">
              <div className="flex flex-wrap items-baseline justify-between gap-[8px] px-[18px] py-[14px] border-b border-[color:var(--color-border-resting)] bg-[color:var(--color-surface-1)]">
                <h2 className="text-[15px] font-[650]">{r.name}</h2>
                <span className="text-[12px] tabular-mono text-muted-foreground border border-[color:var(--color-border-resting)] rounded-full px-[10px] py-[4px] bg-[color:var(--color-surface-1)]">{r.users.length} officer{r.users.length !== 1 ? "s" : ""}</span>
              </div>
              <div className="divide-y divide-[color:var(--color-border-resting)]">
                {r.users.map((u) => {
                  ...
                  return (
                    <div key={u.id} className="flex gap-[14px] px-[18px] py-[14px] hover:bg-[color:var(--color-surface-1)] transition">
                      <div className="size-9 rounded-full bg-[color:var(--color-surface-1)] border border-[color:var(--color-border-resting)] grid place-items-center text-[12px] font-bold shrink-0 mt-[2px]">{(u.name ?? u.email ?? "?").slice(0, 1)}</div>
                      <div className="flex-1 min-w-0">
                        <div className="flex flex-wrap items-baseline gap-[8px]">
                          <p className="text-[14px] font-semibold truncate">{u.name ?? u.email}</p>
                          <span className={`text-[11px] font-semibold px-[8px] py-[3px] rounded-full border ${crit ? "bg-[#FFF1F0] text-[#C9190B] border-[#FECACA]" : top.length ? "bg-[#F0FDF4] text-[#0E7A4B] border-[#BBF7D0]" : "bg-[color:var(--color-surface-1)] text-muted-foreground border-[color:var(--color-border-resting)]"}`}>
