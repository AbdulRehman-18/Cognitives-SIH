          ...
          domains.map(([domainName, comps]) => (
            <div key={domainName} className="rounded-[20px] bg-[color:var(--color-surface-1)] border border-[color:var(--color-border-resting)] shadow-[var(--shadow-card)] overflow-hidden">
              <div className="px-[18px] py-[14px] bg-[color:var(--color-canvas)]/50 border-b border-[color:var(--color-border-resting)] flex items-center justify-between">
                ...
                <span className="rounded-full bg-[color:var(--color-surface-1)] border border-[color:var(--color-border-resting)] px-[10px] py-[4px] text-[11px] tabular-mono">{comps.length} competencies</span>
              </div>
              <div className="divide-y divide-[color:var(--color-border-resting)]">
                <div className="hidden md:grid grid-cols-[1fr_140px_90px] gap-[12px] px-[18px] py-[8px] text-[11px] tracking-[0.06em] uppercase font-semibold text-muted-foreground bg-[color:var(--color-surface-1)]">
                  <span>Competency</span><span>Required level</span><span className="text-right">Weight</span>
                </div>
                {comps
                  .sort((a, b) => Number(b.weight) - Number(a.weight))
                  .map((rc, i) => (
                    <div key={i} className="grid grid-cols-1 md:grid-cols-[1fr_140px_90px] gap-[8px] px-[18px] py-[12px] items-center hover:bg-[color:var(--color-surface-1)] transition">
                      <span className="text-small font-medium">{rc.competency.name}</span>
                      <div className="flex items-center gap-[8px]">
                        <div className="flex gap-[3px]">
                          {[1, 2, 3, 4, 5].map((lv) => (
                            <span key={lv} className={`size-[10px] rounded-full border ${lv <= rc.requiredLevel ? "bg-[color:var(--color-accent)] border-[color:var(--color-accent)]" : "bg-[color:var(--color-surface-1)] border-[color:var(--color-border-resting)]"}`} />
                          ))}
                        </div>
                        <span className="num text-[11px] tabular-mono text-muted-foreground">{rc.requiredLevel} / 5</span>
                      </div>
                      <span className="num text-[12px] tabular-mono font-semibold md:text-right"><span className={`inline-flex rounded-full px-[8px] py-[3px] text-[11px] border ${Number(rc.weight) >= 0.85 ? "bg-[color:var(--color-accent)] text-white border-transparent" : Number(rc.weight) >= 0.7 ? "bg-[color:var(--color-accent)]/10 text-[color:var(--color-accent)] border-[color:var(--color-accent)]/20" : "bg-[color:var(--color-surface-1)] border-[color:var(--color-border-resting)]"}`}>{Number(rc.weight).toFixed(2)}</span></span>
                    </div>
                  ))}
              </div>
            </div>
